...
@section('content')
    ...
    <div class="flex items-start justify-between mb-5">
        <div>
            <h1 class="font-black text-xl text-slate-900" style="font-family:'Syne',sans-serif;">Data Pasien</h1>
            <p class="text-xs text-slate-400 mt-0.5">{{ $totalPasien }} total pasien terdaftar</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100">
            {{-- SESUDAH --}}
            <form method="GET" action="{{ route('pasien.index') }}" id="searchForm" class="flex items-center gap-3">
                <div class="relative">
                    ...
                    <input type="text" name="search" id="searchInput" value="{{ request('search') }}"
                        placeholder="Cari nama, kode pasien" autocomplete="off"
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-100 text-[11px] text-slate-400 uppercase tracking-wide">
                    ...
                    <th class="px-4 py-2.5 text-left">Kode Pasien</th>
                    <th class="px-4 py-2.5 text-left">Nama</th>
                    <th class="px-4 py-2.5 text-left">Email</th>
                    <th class="px-4 py-2.5 text-left">Alamat</th>
                    <th class="px-4 py-2.5 text-center">Usia</th>
            ...
            <tbody class="divide-y divide-slate-50">
                @forelse ($pasiens as $i => $pasien)
                    <tr class="hover:bg-slate-50/60 transition">
                        <td class="px-4 py-3 text-slate-400 text-xs">{{ $pasiens->firstItem() + $i }}</td>
                        <td class="px-4 py-3 text-[13px] text-slate-600">{{ $pasien->kode_pasien ?? '-' }}</td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2.5">
                                <div class="w-8 h-8 rounded-lg flex items-center justify-center text-white font-bold text-[11px] flex-shrink-0"
                                    style="background:{{ $pasien->avatar_color ?? '#0EA5A4' }};font-family:'Syne',sans-serif;">
                                    {{ strtoupper(substr($pasien->nama_lengkap, 0, 1) . substr(strrchr($pasien->nama_lengkap, ' '), 1, 1)) }}
                                </div>
                                <div>
                                    <div class="text-[13px] font-medium text-slate-900">{{ $pasien->nama_lengkap }}</div>
                                    <div class="text-[11px] text-slate-400">{{ $pasien->jenis_kelamin }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-slate-500 text-[13px]">{{ $pasien->email ?? '-' }}</td>
                        <td class="px-4 py-3 text-slate-500 text-[13px] max-w-[200px] break-words whitespace-normal">
                            {{ $pasien->alamat }}
                        </td>
                        <td class="px-4 py-3 text-center text-[13px] text-slate-600">{{ $pasien->umur }} th</td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1.5">
                                {{-- Show --}}
                                <button onclick="openShow({{ $pasien->id }})"
                                ...
                                {{-- Delete --}}
                                <button
                                    onclick="openDelete('{{ route('pasien.destroy', $pasien->id) }}', '{{ $pasien->nama_lengkap }}')"
        ...
        <div class="flex items-center justify-between px-4 py-3 border-t border-slate-100">
            ...
            <div class="flex items-center gap-1">
                ...
                @foreach ($pasiens->getUrlRange(1, $pasiens->lastPage()) as $pg => $url)
                    @if ($pg == $pasiens->currentPage())
                        <span
    ...
    <div id="deleteModal" class="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center hidden"
        ...
        <div class="bg-white rounded-2xl border border-slate-200 w-[400px] overflow-hidden"
            onclick="event.stopPropagation()">
            <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                <span class="font-black text-base text-red-600" style="font-family:'Syne',sans-serif;">Hapus Pasien</span>
            ...
            <div class="p-5">
                <div class="flex items-start gap-3 bg-red-50 border border-red-100 rounded-xl p-3.5 mb-1">
                    ...
                    <p class="text-[13px] text-red-800 leading-relaxed">
                        Yakin ingin menghapus pasien <strong id="deleteNama" class="text-red-700"></strong>?
                        Semua data terapi terkait juga akan terhapus. Tindakan ini tidak dapat dibatalkan.
                    </p>
                </div>
            </div>
    ...
    @push('scripts')
        <script>
            const pasienData = @json($pasiens->items());
            ...
            function openShow(id) {
                ...
                const rows = [
                    ...
                    ['Alamat', p.alamat],
                    ['Kode Pasien', p.kode_pasien],
                ];

                document.getElementById('showBody').innerHTML = `
        <div class="flex items-center gap-3 mb-4 pb-4 border-b border-slate-100">
            <div class="w-11 h-11 rounded-xl flex items-center justify-center text-white font-black text-base flex-shrink-0" style="background:#0EA5A4;font-family:'Syne',sans-serif;">${initials}</div>
            <div class="flex-1 min-w-0">
                <div class="font-black text-[15px] text-slate-900" style="font-family:'Syne',sans-serif;">${p.nama_lengkap}</div>
                <div class="text-[12px] text-slate-400 mt-0.5">${p.jenis_kelamin} · ${p.umur} tahun</div>
            </div>
        </div>
